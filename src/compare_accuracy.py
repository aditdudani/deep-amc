...
ADAPTIVE_JSON = 'results_local/adaptive_sampling/adaptive_v1/accuracy_by_snr_squeezenet.json'
OUT_PATH = 'results_local/compare/accuracy_vs_snr.png'
...
snr_a, acc_a = load(ADAPTIVE_JSON)

plt.figure(figsize=(7.5, 4.5))
plt.plot(snr_b, acc_b, marker='o', label='Baseline SqueezeNet')
plt.plot(snr_g, acc_g, marker='o', label='Baseline GoogLeNet')
plt.plot(snr_a, acc_a, marker='o', label='Adaptive SqueezeNet')
plt.xlabel('SNR (dB)')
plt.ylabel('Accuracy')
plt.title('Accuracy vs SNR: Baseline SqueezeNet vs Baseline GoogLeNet vs Adaptive SqueezeNet')
plt.grid(alpha=0.3)
plt.ylim(0.35, 1.0)
plt.legend()
plt.tight_layout()
