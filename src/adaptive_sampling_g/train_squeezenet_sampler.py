...
"""Train SqueezeNet with (optionally) adaptive sampler for Config G dataset.
...
Run examples:
    ...
    python src/adaptive_sampling_g/train_squeezenet_sampler.py --mode sampler-uniform

    python src/adaptive_sampling_g/train_squeezenet_sampler.py --mode adaptive --warmup-epochs 3 --min-val-acc 0.15
"""
...
from adaptive_sampling_g.sampler import (
    WeightedSamplerSequence,
    init_uniform_weights,
    load_metadata_csv,
    build_buckets,
)
from adaptive_sampling_g.callbacks_confusion_snr import ConfusionBySNRCallback
...
# --------------------
DATA_DIR = os.path.join('data', 'processed_g')
...
RESULTS_DIR = os.path.join('results', 'adaptive_sampling_g', RUN_TAG)
MODEL_OUT = os.path.join('models', f'squeezenet_sampler_g_{RUN_TAG}.h5')
