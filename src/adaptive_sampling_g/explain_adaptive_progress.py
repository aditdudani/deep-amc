...
Inputs: results/adaptive_sampling_g/<RUN_TAG>/ with weights_epoch*.json and confusion_epoch*.json
Output: prints epoch-to-epoch focus shifts and resulting accuracy shifts, and writes a summary file.
Run:
    python src/adaptive_sampling_g/explain_adaptive_progress.py --dir results/adaptive_sampling_g/<RUN_TAG>
If --dir is omitted, it will default to results/adaptive_sampling_g and pick the latest timestamped folder.
"""
...
def main():
    ...
    ap.add_argument('--dir', default=os.path.join('results', 'adaptive_sampling_g'), help='Run directory or parent folder')
