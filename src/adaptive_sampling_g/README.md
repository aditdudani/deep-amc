 # Adaptive Sampling Pipeline (Config G)

This folder is a drop-in clone of the adaptive sampling pipeline, retargeted for **Config G (K20 3x3 Cross Centered)**.
It uses separate paths so experiments do not mix with the original pipeline:
- Images: `data/processed_g`
- Metadata: `data/processed_g/metadata_{train,val}.csv`
- Results: `results/adaptive_sampling_g/<RUN_TAG>`
- Model: `models/squeezenet_sampler_g_<RUN_TAG>.h5`
 ...
 ```bash
 # 1. Parity baseline
 python src/adaptive_sampling_g/train_squeezenet_sampler.py --mode parity
 ...
 python src/adaptive_sampling_g/train_squeezenet_sampler.py ... --uniform-scope class_snr
 ```
 ...
 Use:
 ```bash
 python src/adaptive_sampling_g/inspect_weights.py \
	 --dir results/adaptive_sampling_g/<RUN_TAG> --top 12 --entropy
 ```
 ...
 | Artifact | Path | Notes |
 |----------|------|-------|
| Weights | `results/adaptive_sampling_g/<RUN_TAG>/weights_epoch*.json` | Distribution snapshots |
| Confusion | `results/adaptive_sampling_g/<RUN_TAG>/confusion_epoch*.json` | Per-SNR confusion matrices |
| Logs | `results/adaptive_sampling_g/<RUN_TAG>/logs/` | TensorBoard events |
| CSV | `results/adaptive_sampling_g/<RUN_TAG>/squeezenet_sampler_train_log.csv` | Epoch metrics |
| Model | `models/squeezenet_sampler_g_<RUN_TAG>.h5` | Best by val accuracy |
 
 ## Quick Inspection Shortcuts
 ```bash
 # Latest weights only (shell trick)
 python src/adaptive_sampling_g/inspect_weights.py --dir results/adaptive_sampling_g/<RUN_TAG> --top 12 --entropy | tail -n +1
 
 # Watch weights evolve (simple loop)
 watch -n 30 "python src/adaptive_sampling_g/inspect_weights.py --dir results/adaptive_sampling_g/<RUN_TAG> --top 8 --entropy | grep Epoch | tail -n 5"
 ```
...
Run
- Generate Config G images: `python src/adaptive_sampling_g/preprocess_config_g.py`
- Build metadata: `python src/adaptive_sampling_g/dataset_metadata.py`
- Train: `python src/adaptive_sampling_g/train_squeezenet_sampler.py --mode adaptive --sampler-backend tfdata --uniform-scope class --warmup-epochs 3 --min-val-acc 0.15 --resample-each-epoch`
